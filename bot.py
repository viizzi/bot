...
def send_welcome(message):
    ...
    bot.send_photo(message.chat.id, photo, f"Привет {first_name}✌\n"
                                           f"Я могу скачать музыку с юутуба\n"
                                           f"Пришли мне ссылку на плейлист или видео с YouTube!")
...
def get_files(message):
    ...
    if message.text[:38] == 'https://www.youtube.com/playlist?list=':
        ...
        for url in playlist:
            ...
            bot.send_message(message.chat.id, f"ООО НЕТ!!!\n"
                                              f"Походу плейлист закрыт или вы вили неверную ссылку.")
